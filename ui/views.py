...
def run_manage_clients(db_client: SupabaseClient):
    layout = [
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, 'back_to_main'):
            ...
            _run_register_client_form(db_client)
        elif event == 'list_clients':
            _run_list_clients_table(db_client)

    window.close()
    return next_window


def _run_register_client_form(db_client: SupabaseClient):
    layout = [
...
def _run_list_clients_table(db_client: SupabaseClient):
