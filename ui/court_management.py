...
def run_manage_courts(court_service):
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, 'back_to_main'):
            ...
            _run_register_court_form(court_service)
...
def _run_register_court_form(court_service):
    """
    ...
    while True:
        layout = [
            ...
            [sg.Text('Nome:', size=styles.INPUT_LABEL_SIZE), sg.Input(
                key='name', size=(10, 1), default_text=values.get('name', ''))],
            [sg.Text('Tipo:', size=styles.INPUT_LABEL_SIZE), sg.Combo(court_types, key='court_type',
                                                                      default_value=values.get('court_type', court_types[0]), readonly=True, size=(10, 1))],
            [sg.Text('Descrição:', size=styles.INPUT_LABEL_SIZE), sg.Multiline(
                key='description', size=(35, 3), default_text=values.get('description', ''))],
            [sg.Text('Capacidade:', size=styles.INPUT_LABEL_SIZE), sg.Input(
                key='capacity', size=(5, 1), default_text=values.get('capacity', ''))],
            [sg.Text('Preço/Hora:', size=styles.INPUT_LABEL_SIZE), sg.Input(
                key='price_per_hour', size=(5, 1), default_text=values.get('price_per_hour', ''))],
